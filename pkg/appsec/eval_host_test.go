package appsec

import (
	"context"
	"encoding/json"
	"errors"
	"net/http"
	"net/http/httptest"
	"testing"

	"github.com/wzagrajcz/AkamaiOPEN-edgegrid-golang/v3/pkg/session"
	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

func TestAppSec_ListEvalHost(t *testing.T) {

	result := GetEvalHostResponse{}

	respData := compactJSON(loadFixtureBytes("testdata/TestEvalHost/EvalHost.json"))
	json.Unmarshal([]byte(respData), &result)

	tests := map[string]struct {
		params           GetEvalHostRequest
		responseStatus   int
		responseBody     string
		expectedPath     string
		expectedResponse *GetEvalHostResponse
		withError        error
		headers          http.Header
	}{
		"200 OK": {
			params: GetEvalHostRequest{
				ConfigID: 43253,
				Version:  15,
			},
			headers: http.Header{
				"Content-Type": []string{"application/json"},
			},
			responseStatus:   http.StatusOK,
			responseBody:     string(respData),
			expectedPath:     "/appsec/v1/configs/43253/versions/15/selected-hostnames/eval-hostnames",
			expectedResponse: &result,
		},
		"500 internal server error": {
			params: GetEvalHostRequest{
				ConfigID: 43253,
				Version:  15,
			},
			headers:        http.Header{},
			responseStatus: http.StatusInternalServerError,
			responseBody: `
{
    "type": "internal_error",
    "title": "Internal Server Error",
    "detail": "Error fetching EvalHost",
    "status": 500
}`,
			expectedPath: "/appsec/v1/configs/43253/versions/15/selected-hostnames/eval-hostnames",
			withError: &Error{
				Type:       "internal_error",
				Title:      "Internal Server Error",
				Detail:     "Error fetching EvalHost",
				StatusCode: http.StatusInternalServerError,
			},
		},
	}

	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			mockServer := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				assert.Equal(t, test.expectedPath, r.URL.String())
				assert.Equal(t, http.MethodGet, r.Method)
				w.WriteHeader(test.responseStatus)
				_, err := w.Write([]byte(test.responseBody))
				assert.NoError(t, err)
			}))
			client := mockAPIClient(t, mockServer)
			result, err := client.GetEvalHost(
				session.ContextWithOptions(
					context.Background(),
					session.WithContextHeaders(test.headers),
				),
				test.params)
			if test.withError != nil {
				assert.True(t, errors.Is(err, test.withError), "want: %s; got: %s", test.withError, err)
				return
			}
			require.NoError(t, err)
			assert.Equal(t, test.expectedResponse, result)
		})
	}
}

// Test EvalHost
func TestAppSec_GetEvalHost(t *testing.T) {

	result := GetEvalHostResponse{}

	respData := compactJSON(loadFixtureBytes("testdata/TestEvalHost/EvalHost.json"))
	json.Unmarshal([]byte(respData), &result)

	tests := map[string]struct {
		params           GetEvalHostRequest
		responseStatus   int
		responseBody     string
		expectedPath     string
		expectedResponse *GetEvalHostResponse
		withError        error
	}{
		"200 OK": {
			params: GetEvalHostRequest{
				ConfigID: 43253,
				Version:  15,
			},
			responseStatus:   http.StatusOK,
			responseBody:     respData,
			expectedPath:     "/appsec/v1/configs/43253/versions/15/selected-hostnames/eval-hostnames",
			expectedResponse: &result,
		},
		"500 internal server error": {
			params: GetEvalHostRequest{
				ConfigID: 43253,
				Version:  15,
			},
			responseStatus: http.StatusInternalServerError,
			responseBody: (`
{
    "type": "internal_error",
    "title": "Internal Server Error",
    "detail": "Error fetching EvalHost"
}`),
			expectedPath: "/appsec/v1/configs/43253/versions/15/selected-hostnames/eval-hostnames",
			withError: &Error{
				Type:       "internal_error",
				Title:      "Internal Server Error",
				Detail:     "Error fetching EvalHost",
				StatusCode: http.StatusInternalServerError,
			},
		},
	}

	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			mockServer := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				assert.Equal(t, test.expectedPath, r.URL.String())
				assert.Equal(t, http.MethodGet, r.Method)
				w.WriteHeader(test.responseStatus)
				_, err := w.Write([]byte(test.responseBody))
				assert.NoError(t, err)
			}))
			client := mockAPIClient(t, mockServer)
			result, err := client.GetEvalHost(context.Background(), test.params)
			if test.withError != nil {
				assert.True(t, errors.Is(err, test.withError), "want: %s; got: %s", test.withError, err)
				return
			}
			require.NoError(t, err)
			assert.Equal(t, test.expectedResponse, result)
		})
	}
}

// Test Update EvalHost.
func TestAppSec_UpdateEvalHost(t *testing.T) {
	result := UpdateEvalHostResponse{}

	respData := compactJSON(loadFixtureBytes("testdata/TestEvalHost/EvalHost.json"))
	json.Unmarshal([]byte(respData), &result)

	req := UpdateEvalHostRequest{}

	reqData := compactJSON(loadFixtureBytes("testdata/TestEvalHost/EvalHost.json"))
	json.Unmarshal([]byte(reqData), &req)

	tests := map[string]struct {
		params           UpdateEvalHostRequest
		responseStatus   int
		responseBody     string
		expectedPath     string
		expectedResponse *UpdateEvalHostResponse
		withError        error
		headers          http.Header
	}{
		"200 Success": {
			params: UpdateEvalHostRequest{
				ConfigID: 43253,
				Version:  15,
			},
			headers: http.Header{
				"Content-Type": []string{"application/json;charset=UTF-8"},
			},
			responseStatus:   http.StatusCreated,
			responseBody:     respData,
			expectedResponse: &result,
			expectedPath:     "/appsec/v1/configs/43253/versions/15/selected-hostnames/eval-hostnames",
		},
		"500 internal server error": {
			params: UpdateEvalHostRequest{
				ConfigID: 43253,
				Version:  15,
			},
			responseStatus: http.StatusInternalServerError,
			responseBody: (`
{
    "type": "internal_error",
    "title": "Internal Server Error",
    "detail": "Error creating EvalHost"
}`),
			expectedPath: "/appsec/v1/configs/43253/versions/15/selected-hostnames/eval-hostnames",
			withError: &Error{
				Type:       "internal_error",
				Title:      "Internal Server Error",
				Detail:     "Error creating EvalHost",
				StatusCode: http.StatusInternalServerError,
			},
		},
	}

	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			mockServer := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				assert.Equal(t, http.MethodPut, r.Method)
				w.WriteHeader(test.responseStatus)
				if len(test.responseBody) > 0 {
					_, err := w.Write([]byte(test.responseBody))
					assert.NoError(t, err)
				}
			}))
			client := mockAPIClient(t, mockServer)
			result, err := client.UpdateEvalHost(
				session.ContextWithOptions(
					context.Background(),
					session.WithContextHeaders(test.headers)), test.params)
			if test.withError != nil {
				assert.True(t, errors.Is(err, test.withError), "want: %s; got: %s", test.withError, err)
				return
			}
			require.NoError(t, err)
			assert.Equal(t, test.expectedResponse, result)
		})
	}
}
